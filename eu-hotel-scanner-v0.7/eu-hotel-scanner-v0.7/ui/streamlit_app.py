...
def main():
    st.set_page_config(
        page_title="EU Hotel Scanner – v0.7",
        layout="wide",
    )

    st.title("EU Hotel Scanner – v0.7")
    st.caption(
        "Multi-vendor aggregation + simple response caching. Vendors are configured "
        "via config/vendors.yaml, with live HTTP clients using a shared file-based cache."
    )
    ...
    if not run_btn:
        st.info(
            ...
            "Vendors and caching are controlled via config/vendors.yaml."
        )
    else:
        if checkin >= checkout:
        ...
        with st.spinner("Running scan (multi-vendor, cached where configured)..."):
