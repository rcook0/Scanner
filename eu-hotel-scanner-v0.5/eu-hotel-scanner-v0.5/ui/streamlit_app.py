...
def main():
    st.set_page_config(
        page_title="EU Hotel Scanner – v0.5",
        layout="wide",
    )

    st.title("EU Hotel Scanner – v0.5")
    st.caption(
        "Data-quality focused iteration: rating/star filters, normalized prices via FX, "
        "and richer per-country metrics on top of the cost-index + logging engine."
    )
    ...
    if not run_btn:
        st.info("Adjust parameters in the sidebar and click **Run scan** to log a new run.")
    else:
        if checkin >= checkout:
