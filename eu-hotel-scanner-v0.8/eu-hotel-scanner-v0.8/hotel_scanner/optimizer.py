...
def build_country_scan_weights(
    ...
    # Extract raw weights for scaling
    raw_non_zero = [e.raw_weight for e in non_zero_entries if e.raw_weight > 0]
    if not raw_non_zero:
        # Everyone is equal; either scan nothing or give them all weight 1
    ...
    median_val = raw_sorted[median_idx] if median_sorted := raw_sorted else 1.0
    if median_val <= 0:
