...
def main():
    st.set_page_config(
        page_title="EU Hotel Scanner – v0.6",
        layout="wide",
    )

    st.title("EU Hotel Scanner – v0.6")
    st.caption(
        "Real-vendor integration harness. Vendors are configured via config/vendors.yaml "
        "and API keys via .env. UI still behaves like v0.5, but the underlying scan "
        "can now use live APIs via the common client interface."
    )
    ...
    if not run_btn:
        st.info(
            ...
            "Vendors are controlled via config/vendors.yaml."
        )
    else:
        if checkin >= checkout:
        ...
        with st.spinner("Running scan (vendors from config/vendors.yaml)..."):
        ...
        if not metrics_by_country:
            ...
            st.subheader("Sample cheapest offers per country (current run)")
