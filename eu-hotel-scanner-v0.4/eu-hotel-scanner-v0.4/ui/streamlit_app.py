...
def main():
    st.set_page_config(
        page_title="EU Hotel Scanner – v0.4",
        layout="wide",
    )

    st.title("EU Hotel Scanner – v0.4")
    st.caption(
        "Prototype using a country cost index. v0.4 logs each scan to SQLite and "
        "compares observed hotel prices vs the static cost index to flag "
        "systematically cheap/expensive countries."
    )
    ...
    if not run_btn:
        st.info("Adjust the parameters in the sidebar and click **Run scan** to log a new run.")
    else:
        if checkin >= checkout:
        ...
        if not metrics_by_country:
            ...
            st.subheader("Current run – country rankings")
            ...
            st.subheader("Sample cheapest offers per country (current run, mock)")
    ...
    st.subheader("Historical mispricing vs cost index")

    conn_hist = get_connection(DEFAULT_DB_PATH)
    summary = get_historical_country_summary(conn_hist)

    if not summary:
        st.info("No historical data yet. Run at least one scan with logging enabled.")
        return

    # Countries systematically cheaper than their cost index (low normalized_median)
    cheaper = sorted(summary, key=lambda r: r["normalized_median"])
    more_expensive = sorted(summary, key=lambda r: r["normalized_median"], reverse=True)
